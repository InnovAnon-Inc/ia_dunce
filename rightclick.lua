...
function ia_dunce.right_click(self, pos, sneak)
	minetest.log('ia_dunce.rightclick()')
    ...
    if not sneak and def and def.on_rightclick then
        ...
        if new_stack then self.object:set_wielded_item(new_stack) end
        success = true
    else
        -- If sneaking OR node has no right-click, try to place/use the held item
        success = ia_dunce.place(self, pos)
    end
