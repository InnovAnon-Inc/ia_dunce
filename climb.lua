...
function ia_dunce.is_at_climb_up_target(self, target_y)
...
function ia_dunce.is_at_climb_down_target(self, target_y)
    local pos = self.object:get_pos()
    if not pos then return false end
...
function ia_dunce.climb_up(self, target_y)
    local pos = self.object:get_pos()
    if not pos or not ia_dunce.is_climbable(pos) then
    ...
    if not ia_dunce.is_climbable(above) and pos.y >= (target_y - 0.5) then
        local yaw = self.object:get_yaw()
        local dir = {x = -math.sin(yaw), z = math.cos(yaw)}
        -- Vault over the top
        self.object:set_velocity({x = dir.x * 2.2, y = 2.0, z = dir.z * 2.2})
    else
...
function ia_dunce.climb_down(self, target_y)
    local pos = self.object:get_pos()
    if not pos or not ia_dunce.is_climbable(pos) then
        self._is_climbing = false
        return false
    ...
    -- Controlled descent
    self.object:set_velocity({x = v.x, y = -3.0, z = v.z})
...
function ia_dunce.climb(self, target_y)
    ...
    if target_y and target_y < pos.y - 0.5 then
        return ia_dunce.climb_down(self, target_y)
    else
        -- Default to up if no target or target is above
        return ia_dunce.climb_up(self, target_y or (pos.y + 1))
    end
end
